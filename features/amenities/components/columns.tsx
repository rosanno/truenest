...
export const columns: ColumnDef<Aminity>[] = [
    ...
    header: ({ column }) => {
      return (
        <div className="flex items-center gap-2 cursor-pointer hover:text-primary transition-colors duration-200 w-fit" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>
          ...
          {column.getIsSorted() === "asc" ? <ArrowDownAZ className="h-4 w-4 opacity-50 hover:opacity-100 transition-opacity" /> : <ArrowUpZA className="h-4 w-4 opacity-50 hover:opacity-100 transition-opacity" />}
        </div>
      );
    },
  },
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="truncate max-w-[200px] relative group cursor-help" title={description}>
          {truncatedText}
          <div className="absolute z-50 invisible group-hover:visible bg-black text-white text-sm rounded-md p-2 min-w-[200px] max-w-[300px] break-words shadow-lg -mt-1 left-0 transform translate-y-full">{description}</div>
        </div>
      );
    },
  },
  {
    accessorKey: "icon",
    header: () => {
      return <div className="font-semibold">Icon</div>;
    },
  },
  {
    id: "actions",
